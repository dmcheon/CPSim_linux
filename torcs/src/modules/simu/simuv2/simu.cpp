...
ctrlCheck(tCar *car)
{
    ...
    /* sanity check */
#ifndef WIN32
    if (isnan((float)car->ctrl->accelCmd) || isinf((float)car->ctrl->accelCmd)) car->ctrl->accelCmd = 0;
    if (isnan((float)car->ctrl->brakeCmd) || isinf((float)car->ctrl->brakeCmd)) car->ctrl->brakeCmd = 0;
    if (isnan((float)car->ctrl->clutchCmd) || isinf((float)car->ctrl->clutchCmd)) car->ctrl->clutchCmd = 0;
    if (isnan((float)car->ctrl->steer) || isinf((float)car->ctrl->steer)) car->ctrl->steer = 0;
