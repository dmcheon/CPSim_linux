...
ReOneStep(double deltaTimeIncrement)
{
	...
	if(torcs_output[CC_TRIGGER] > 0.5 && torcs_output[LKAS_TRIGGER] > 0.5)
		a = a + 2;
	else if(torcs_output[CC_TRIGGER] > 0.5 || torcs_output[LKAS_TRIGGER] > 0.5)
		a = a + 1;

	if(torcs_output[CC_TRIGGER] < 0.5 && torcs_output[LKAS_TRIGGER] > 0.5)
	{
		ReRaceBigMsgSet("Speed Control", 0.1);
	}
	else if(torcs_output[CC_TRIGGER] > 0.5 && torcs_output[LKAS_TRIGGER] < 0.5)
	{
		ReRaceBigMsgSet("Lane Keeping", 0.1);
	}
	else if(torcs_output[CC_TRIGGER] < 0.5 && torcs_output[LKAS_TRIGGER] < 0.5)
	{
		ReRaceBigMsgSet("SC & LK", 0.1);
	}

	// refresh current time.
	torcs_output[PASSED_TIME] = (float)s->currentTime;
	/************/
	...
	/*** kswe ***/
	/* where is _speed_x refreshed? */
	/************/

	START_PROFILE("_reSimItf.update*");
	ReInfo->_reSimItf.update(s, deltaTimeIncrement, -1);	// update dynamics (expected)
	for (i = 0; i < s->_ncars; i++) {
		ReManage(s->cars[i]);
	}
...
ReUpdate(void)
{
	...
	/*** kswe ***/
	// update simulated time rapidly
	if(pre_time != 0.0)
	{
		torcs_output[PASSED_TIME] += (pre_time-t);
		pre_time = t;
	}
	/************/
	...
	/*** kswe ***/
	/* if enough time passed, update data */
	/************/
