...
#define DFWD 1
#define D4WD 2
#define N_TRACK_FORESEE 20	//get this number of track segment including current segment
...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	/*** kswe ***/
	// CC trigger
	if (((cmd[CMD_SCC].type == GFCTRL_TYPE_JOY_BUT) && joyInfo->edgeup[cmd[CMD_SCC].val]) ||
	((cmd[CMD_SCC].type == GFCTRL_TYPE_KEYBOARD) && keyInfo[cmd[CMD_SCC].val].edgeUp) ||
	((cmd[CMD_SCC].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_SCC].val].edgeUp))
	{
		HCtx[idx]->ParamScc = 1 - HCtx[idx]->ParamScc;
	}
	torcs_output[CC_TRIGGER] = HCtx[idx]->ParamScc;

	// LKAS trigger
	if (((cmd[CMD_LKAS].type == GFCTRL_TYPE_JOY_BUT) && joyInfo->edgeup[cmd[CMD_LKAS].val]) ||
	((cmd[CMD_LKAS].type == GFCTRL_TYPE_KEYBOARD) && keyInfo[cmd[CMD_LKAS].val].edgeUp) ||
	((cmd[CMD_LKAS].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_LKAS].val].edgeUp))
	{
		HCtx[idx]->ParamLkas = 1 - HCtx[idx]->ParamLkas;
	}
	torcs_output[LKAS_TRIGGER] = HCtx[idx]->ParamLkas;
	...
	float track_num = 0.0;
	float track_type[N_TRACK_FORESEE];	//get type of track segment. 1: Right curve, 2: Left curve, 3: Straight
	float track_radius[N_TRACK_FORESEE];	//Radius of track segment in meters of the middle of the track
	float track_friction[N_TRACK_FORESEE];	//friction value of track segment

	tTrackSeg* seg = car->_trkPos.seg;

	// LKAS
	{
		// normal steering (based on current position only)
		track_angle = RtTrackSideTgAngleL(&(car->_trkPos));
		dist = car->_trkPos.toMiddle;
		for(int segIndex = 0; segIndex < N_TRACK_FORESEE; segIndex++)
		{
			track_type[segIndex] = seg->type;
			track_radius[segIndex] = seg->radius;
			track_friction[segIndex] = seg->surface->kFriction;	
			seg = seg->next;
		}
		
		//dmcheon
		if (strcmp(track_name, "wheel-1") == 0) track_num = 0.1;
		else if (strcmp(track_name, "wheel-2") == 0) track_num = 0.2;
		else if (strcmp(track_name, "aalborg") == 0) track_num = 0.3;
		else if (strcmp(track_name, "alpine-1") == 0) track_num = 0.4;
		else if (strcmp(track_name, "alpine-2") == 0) track_num = 0.5;
		else if (strcmp(track_name, "e-track-1") == 0) track_num = 0.6;
		else if (strcmp(track_name, "e-track-2") == 0) track_num = 0.7;
		else if (strcmp(track_name, "e-track-3") == 0) track_num = 0.8;
		else if (strcmp(track_name, "e-track-4") == 0) track_num = 0.9;
		else if (strcmp(track_name, "e-track-6") == 0) track_num = 1.0;
		else if (strcmp(track_name, "eroad") == 0) track_num = 1.1;
		else if (strcmp(track_name, "forza") == 0) track_num = 1.2;
		else if (strcmp(track_name, "g-track-1") == 0) track_num = 1.3;
		else if (strcmp(track_name, "g-track-2") == 0) track_num = 1.4;
		else if (strcmp(track_name, "g-track-3") == 0) track_num = 1.5;
		else if (strcmp(track_name, "ole-road-1") == 0) track_num = 1.6;
		else if (strcmp(track_name, "ruudskogen") == 0) track_num = 1.7;
		else if (strcmp(track_name, "spring") == 0) track_num = 1.8;
		else if (strcmp(track_name, "street-1") == 0) track_num = 1.9;
		else if (strcmp(track_name, "a-speedway") == 0) track_num = 2.1;
		else if (strcmp(track_name, "b-speedway") == 0) track_num = 2.2;
		else if (strcmp(track_name, "c-speedway") == 0) track_num = 2.3;
		else if (strcmp(track_name, "d-speedway") == 0) track_num = 2.4;
		else if (strcmp(track_name, "e-speedway") == 0) track_num = 2.5;
		else if (strcmp(track_name, "f-speedway") == 0) track_num = 2.6;
		else if (strcmp(track_name, "g-speedway") == 0) track_num = 2.7;
		else if (strcmp(track_name, "michigan") == 0) track_num = 2.8;
		else if (strcmp(track_name, "e-track-5") == 0) track_num = 2.9;
		else if (strcmp(track_name, "dirt-1") == 0) track_num = 3.1;
		else if (strcmp(track_name, "dirt-2") == 0) track_num = 3.2;
		else if (strcmp(track_name, "dirt-3") == 0) track_num = 3.3;
		else if (strcmp(track_name, "dirt-4") == 0) track_num = 3.4;
		else if (strcmp(track_name, "dirt-5") == 0) track_num = 3.5;
		else if (strcmp(track_name, "dirt-6") == 0) track_num = 3.6;
		else if (strcmp(track_name, "mixed-1") == 0) track_num = 3.7;
		else if (strcmp(track_name, "mixed-2") == 0) track_num = 3.8;
	}

	torcs_output[SPEED] = 3.6*car->_speed_x;
//	torcs_output[STEER_VALUE] = angle/car->_steerLock;
	...
	torcs_output[TRACK_NUM] = track_num;
	
	for(int segIndex = 0; segIndex < N_TRACK_FORESEE; segIndex++)
	{
		torcs_output[TRACK_TYPE + segIndex] = track_type[segIndex];
		torcs_output[TRACK_RADIUS + segIndex] = track_radius[segIndex];
		torcs_output[TRACK_FRICTION + segIndex] = track_friction[segIndex];
	}
	/*** kswe ***/
